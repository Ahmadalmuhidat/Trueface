...
def GetStudentsByLecture(request):
  if request.method == "GET":
    ...
    relations = ClassStudentRelation.objects.filter(
  ...
  if request.method == "POST":
    try:
      ...
      try:
        ...
        relation = ClassStudentRelation.objects.filter(
          student_id=student_id,
          class_field=class_obj,
          day=day
        ).first()
        ...
        if relation:
          ...
          return JsonResponse({"error": "Student not found in this class"}, status=404)
