...
class Users():
  ...
  def _display_users_table(self):
    ...
    for row, user in enumerate(self._context.get_users(), start=1):
      self._add_row(user, row)

    self.users_count.configure(text="Results: " + str(len(self._context.get_users())))
    self._config.loading_cursor_off()
  ...
  def _submit(self):
    ...
    self._add_row(new_user, len(self._context.get_users()) + 1)
